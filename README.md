...
The development of the Quality Auditing App was successful in achieving its primary objective of standardising data collection. A key strength of the project was the strict separation between the backend processing logic and the frontend user interface. This modular architecture not only improved overall code maintainability but also ensured that the core processing rules remained isolated. Designing the frontend to dynamically generate the audit form directly from the underlying CSV database proved to be a highly effective solution. This approach allows operational teams to seamlessly update or add new audit criteria within the spreadsheet without requiring any modifications to the Python codebase. Because the core scoring logic relies on pure functions, the system is inherently designed to support straightforward, repeatable automated testing in isolation from the graphical user interface. Finally, initial review and feedback gathered from operational management indicated that the prototype successfully met the business requirements, with the application being described as intuitive and easy to use for recording quality scores.

During the development phase, a significant technical challenge emerged regarding the handling of null values within the CSV dataset, highlighting an area where the initial data pipeline could have been improved. Several quality criteria questions did not require an "N/A" option, which resulted in empty strings being passed to the frontend, causing the Streamlit framework to render broken, blank radio buttons. This issue was effectively resolved by adapting the object-oriented design to include a list comprehension within the initialisation of the `QualityCriterion` class. By implementing `[opt for opt in [opt_yes, opt_no, opt_na] if opt]`, the application dynamically filters out empty strings. Additionally, the software architecture evolved from the initial design. While the original UML diagram included a `total_score` attribute within the `QualityController`, storing this permanently proved unnecessarily complex. The code was instead updated to calculate the score dynamically using pure functions, which simplified the logic and improved overall testability.

A final area for reflection involves the choice of data storage format. While the current CSV solution successfully meets the requirement for persistent data storage, a more integrated version would utilize the Google Sheets API to sync directly with existing team dashboards. Implementing this was considered out of scope for this MVP, as it would have required managing complex API credentials and authentication for what is primarily a concept test. Instead, the priority was to prove the underlying programmatic logic and establish a robust, testable framework. This approach ensures that the core mechanics are validated before the organization transitions the tool to a more permanent, fully integrated enterprise platform.

In conclusion, this project provides a successful demonstration of how a manual spreadsheet-based process can be modernised into a structured, centralised auditing system. As a minimum viable product, the application effectively proves the feasibility of dynamic data entry and automated scoring logic. However, for a permanent workplace solution, the system would need to be scaled to meet stricter internal IT requirements, particularly regarding user authentication and secure data hosting. Transitioning the core logic from this Python prototype into an enterprise platform like AppSheet would likely be the most viable path forward. This would enable the team to maintain the improved workflow and data consistency established during this project while ensuring full compliance with organisational standards for data security and long-term maintainability.